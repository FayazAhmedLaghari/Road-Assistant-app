...
class _ServiceProvideState extends State<ServiceProvide> {
  ...
  void _fetchSelectedServices() async {
    QuerySnapshot snapshot = await FirebaseFirestore.instance.collection('selectedServices').get();
    setState(() {
      activeServices = snapshot.docs.map((doc) => doc['service'] as String).toList();
    });
  }

  void saveToFirestore() {
    if (selectedServiceType != null && selectedService != null && !activeServices.contains(selectedService)) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          const Text("Service Provide For", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
          ...
          const SizedBox(height: 20),
          const Text("Your Service", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
          const SizedBox(height: 10),
          Expanded(
            child: GridView.count(
              ...
              children: (serviceOptions[selectedServiceType] ?? [])
                  .map((service) => _serviceCard(service["name"], service["icon"]))
                  .toList(),
            ),
          ),
        ],
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 10),
              Icon(isSelected ? Icons.check_circle : Icons.cancel, color: isSelected ? Colors.green : Colors.red),
            ],
          ),
        ),
      ),
