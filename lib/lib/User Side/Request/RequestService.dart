...
class _RequestServiceScreenState extends State<RequestServiceScreen> {
  ...
  Widget _buildVehicleOption(String vehicle, String imagePath) {
    return ListTile(
      leading: Image.asset(imagePath, width: 50, height: 50, fit: BoxFit.contain),
      title: Text(vehicle, style: const TextStyle(fontWeight: FontWeight.bold)),
      onTap: () {
        setState(() {
          selectedVehicle = vehicle;
        });
        FirebaseFirestore.instance.collection('userSelectedVehicle').doc('currentVehicle').set({'vehicle': vehicle});
  ...
  void fetchServices(String vehicleType) async {
    ...
    QuerySnapshot querySnapshot = await FirebaseFirestore.instance.collection('selectedServices').get();
    List<Map<String, dynamic>> fetchedServices = querySnapshot.docs.map((doc) => {
      "name": doc["service"],
  ...
  void saveSelectedService() async {
    if (selectedService != null) {
      await FirebaseFirestore.instance.collection('userSelectedService').doc('currentService').set({
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Expanded(child: isLoading ? _buildShimmerEffect() : _buildServiceGrid()),
                  const SizedBox(height: 16.0),
                  _buildConfirmButton(),
                ],
              ),
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(service['name'], textAlign: TextAlign.center, style: const TextStyle(color: Colors.black)),
              ],
            ),
          ),
        );
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        style: ElevatedButton.styleFrom(
          backgroundColor: selectedService != null ? const Color(0xFF001E62) : Colors.grey,
        ...
        onPressed: selectedService != null
            ? () {
                saveSelectedService();
                Navigator.push(context, MaterialPageRoute(builder: (context) => RequestConfirmation()));
              }
            : null,
        child: const Text('Confirm Issue', style: TextStyle(fontSize: 16.0, color: Colors.white)),
      ),
    );
  }
